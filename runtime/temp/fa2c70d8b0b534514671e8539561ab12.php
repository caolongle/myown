<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:90:"E:\phpstudy\PHPTutorial\WWW\crm_gk\public/../application/scxfx_gk\view\user\edit_role.html";i:1516860069;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
        ...
        <div class="col-sm-12">
            <div class="ibox float-e-margins">
                <div class="ibox-content">
                    <form method="post" action='/scxfx_gk/user/edit_role_info' class="form-horizontal" onsubmit="return check()">
                        <div class="form-group">
                            <label class="col-sm-2 control-label">角色名称：</label>
                            <div class="col-sm-10">
                                <input  type="text"  class="form-control role_name" name="role_name" value="<?php echo $role['r_name']; ?>" />
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                <select name="parent_id" class="form-control"><option value="0">最高级</option></if>
                                    <?php if(is_array($roles) || $roles instanceof \think\Collection || $roles instanceof \think\Paginator): $i = 0; $__LIST__ = $roles;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?>
                                        <option value="<?php echo $val['id']; ?>" <?php if($val['id'] == $role['parent_id']): ?>selected="selected"<?php endif; ?>><?php echo $val['r_name']; ?></option>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-10 role_pri">
                                ...
                                <?php echo str_repeat('&nbsp', 6*$v['level']); ?>
                                    <input type="checkbox" value="<?php echo $v['id']; ?>" name="pri_id[]" class="pri" id="pri<?php echo $v['id']; ?>" level="<?php echo $v['level']?>" style="margin:5px;" <?php if(in_array($v['id'],$pids)):?> checked="checked"<?php endif;?>>
                        ...
                        <input type="hidden" value="<?php echo $role['id']; ?>" name="rid" />
                    </form>
                </div>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    $('input[name=role_name]').blur(function(){
        ...
        if(r_name){
            $.post('/scxfx_gk/user/check_rname',{r_name:r_name,id:id},function(data){
                if(data.state == 1){
