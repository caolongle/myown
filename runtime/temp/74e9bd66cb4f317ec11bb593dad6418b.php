<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:90:"E:\phpstudy\PHPTutorial\WWW\crm_gk\public/../application/scxfx_gk\view\user\user_list.html";i:1517297882;s:47:"../application/scxfx_gk/view/common/header.html";i:1517382407;s:45:"../application/scxfx_gk/view/common/body.html";i:1517383856;s:49:"../application/scxfx_gk/view/common/resource.html";i:1517194925;}*/ ?>
<!DOCTYPE html>
<html>
<head>
	    <meta charset="utf-8" />
	...
	<div class="col-xs-10 pull-right">
		...
		<div class="col-xs-3" style="line-height: 34px;">
			...
			<select style="width:65%;float: left;height:34px;margin-left: 15px;" class="gender" name="gender">
				<option value="-1">请选择</option>
				<option value="0" <?php if($gender == 0): ?>selected="selected"<?php endif; ?>>男</option>
				<option value="1" <?php if($gender == 1): ?>selected="selected"<?php endif; ?>>女</option>
			</select>
		</div>
				...
				<div class="row">
							...
							<table id="sample-table-1" class="table table-striped table-bordered table-hover">
								<thead>
									<tr>
										...
										<th class="hidden-480">
											<i class="icon-mars bigger-110 hidden-480"><img src="/static/img/gender.png"/></i>
											性别
										</th>
										<th>
											<i class="bigger-110 hidden-480"><img src="/static/img/actor.png"/></i>
											所在角色
										</th>
										<th class="hidden-480">
								...
								<tbody>
									<?php if(is_array($user) || $user instanceof \think\Collection || $user instanceof \think\Paginator): $i = 0; $__LIST__ = $user;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?>
										<tr>
											...
											<td><?php echo $val['lead_number']; ?></td>
											<td><?php echo $val['number']; ?></td>

											<td><?php echo $val['name']; ?></td>

											<td>
												<span class="label label-primary arrowed-right"><?php echo $val['tel']; ?></span>
											</td>

											<td class="hidden-480"><?php if($val['gender'] == 0): ?>男<?php else: ?>女<?php endif; ?></td>

											<td>
												<span class="label label-warning arrowed-in arrowed-in-right"><?php echo $val['r_name']; ?></span>
											</td>

											<td class="hidden-480"><?php echo date('Y-m-d',$val['create_time']); ?></td>

											<td>
												<span class="label label-success arrowed"><?php if($val['last_time'] == 0): ?>新用户，未登录<?php else: ?><?php echo date('Y-m-d H:i:s',$val['last_time']); endif; ?></span>
											</td>
											<td class="hidden-480"><?php if($val['last_ip'] == null): ?>新用户，未登录<?php else: ?><?php echo $val['last_ip']; endif; ?></td>
											<td>
											

												<div class="visible-md visible-lg hidden-sm hidden-xs action-buttons">												
														<a class="green iframe" value="/scxfx_gk/user/edit_user/id/<?php echo $val['id']; ?>" style="cursor: pointer" cate="1">
												...
												<div class="visible-xs visible-sm hidden-md hidden-lg">
													<div class="inline position-relative">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow pull-right dropdown-caret dropdown-close">
															...
															<li>
																<a class="tooltip-success iframe" data-rel="tooltip" title="编辑" value="/scxfx_gk/user/edit_user/id/<?php echo $val['id']; ?>" cate="1">
																	<span class="green">
															...
															<li>
																<a class="tooltip-error del" data-rel="tooltip" title="离职" onclick="del(<?php echo $val['id']; ?>)">
																	<span class="red">
					...
					<div class="col-xs-11" style="position: relative">
						...
						<div style="float: left">
							<button class="btn btn-sm btn-pink iframe" value="/scxfx_gk/user/add_user" cate="2">
...
//				move: false,
		...
		$('#team').change(function(){
			...
			if(team != -1){
				$('#sub_team').empty();
				var str = '';
				$.post('/scxfx_gk/user/get_sub_team',{team:team},function(data){
					if(data.state == 1){
        ...
        $('.quit').click(function(){
            ...
            if(idArr != ''){
                layer.confirm('你确定对所选对象进行离职操作?', {icon: 3, title:'离职'}, function(){
                    $.post('/scxfx_gk/user/user_quit_batch',{idArr:idArr},function(data){
                        if(data.state == 1){
                            layer.msg(data.info, {
                                icon: 1,
    ...
    function del(id){
        layer.confirm('确定该员工已离职?', {icon: 3, title:'删除'}, function(){
            $.post('/scxfx_gk/user/user_quit',{id:id},function(data){
                if(data.state == 1){
					layer.msg(data.info, {
                        icon: 1,
